...
func Update(_delta: float):
	...
	if script_user.in_gather_zone:
		...
		if script_user.gather_tick > script_user.gather_tick_timer:
			...
			# TODO: This is broken, it pretty much overrides the sounds of gunshots entirely Audio.play_sound(load(script_user.current_zone.sound_list[random]), script_user.global_position,-randi_range(-20,-25),250)
